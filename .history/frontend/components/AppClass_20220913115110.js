  ...
  // You can delete them and build your own logic from scratch.
   ...
   getXY() {
    ...
    if(this.state.index === 0){
      setData({...this.state, message: "(1,1)"})
    }
    else if(this.state.index === 1){
      setData({...this.state, message: "(2,1)"})
    }
    else if(this.state.index === 2){
      setData({...this.state, message: "(3,1)"})
    }
    else if(this.state.index === 3){
      setData({...this.state, message: "(1,2)"})
    }
    else if(this.state.index === 4){
      setData({...this.state, message: initialMessage})
    }
    else if(this.state.index === 5){
      setData({...this.state, message: "(3,2)"})
    }
    else if(this.state.index === 6){
      setData({...this.state, message: "(1,3)"})
    }
    else if(this.state.index === 7){
      setData({...this.state, message: "(2,3)"})
    }
    else if(this.state.index === 8){
      setData({...this.state, message: "(3,3)"})
    }
    else{return null}
  }
