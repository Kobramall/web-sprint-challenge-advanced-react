...
export default class AppClass extends React.Component {
  ...
  setX(){
    if(this.state.message === "(1,1)" || this.state.message === "(1,2)" ||this.state.message === "(1,3)" ){
      console.log("HERE")
      return 1
    }
    else if(this.state.message === "(2,1)" || this.state.message === "(2,2)" ||this.state.message === "(2,3)" ){
      return 2
    }
    if(this.state.message === "(3,1)" || this.state.message === "(3,2)" ||this.state.message === "(3,3)" ){
      return 3
    }
  }

  setY(){
    if(this.state.message === "(1,1)" || this.state.message === "(2,1)" ||this.state.message === "(3,1)" ){
      return 1
    }
    else if(this.state.message === "(1,2)" || this.state.message === "(2,2)" ||this.state.message === "(3,2)" ){
      return 2
    }
    if(this.state.message === "(1,3)" || this.state.message === "(2,3)" ||this.state.message === "(3,3)" ){
      return 3
    }
  }
  ...
  reset = () => {
    this.setState({...this.state, index: 4, steps:initialSteps});
  }
  ...
  move = (evt) => {
   ...
   componentDidUpdate(prevProps, prevState) {
    if (prevState.index !== this.state.index) {
      this.setState({...this.state, message: this.getXY(), steps: (this.state.steps + 1), x: this.setX(), y: this.setY()})
